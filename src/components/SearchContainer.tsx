...
export const SearchContainer = ():React.JSX.Element => {
    const {state:{searchResults} , setSearchTerm} = useSearch();
                        ...
                        return(
                            <div key={result?.id}
                              onClick={() => handleNavigate(fullName, slug)}
                                className={
                                    `flex items-center mb-6 cursor-pointer transition duration-500 ease-in-out
                            ${theme === 'lightMode' ? 'hover:bg-gray-300 p-2' : theme === 'darkMode' && 'hover:bg-white-600 p-2'
                                    }
                            `
                                }>
